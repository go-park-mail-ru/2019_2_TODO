...
func TestSelectByLoginAndPassword(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("cant create mock: %s", err)
	}
	...
	expectOutput := &User{
		ID:       elemID,
		Username: login,
		Avatar:   testInput[0].Avatar,
	}

	for _, item := range testInput {
		rowResult = rowResult.AddRow(item.ID, item.Username, item.Avatar)
	}

	mock.
		ExpectQuery("SELECT id, login, avatar FROM users WHERE").
	...
	item, err := repo.SelectByLoginAndPassword(elemSearch)
	...
	if !reflect.DeepEqual(item, expectOutput) {
		t.Errorf("results not match, want %v, have %v", expectOutput, item)
		return
	}
	...
	_, err = repo.SelectByLoginAndPassword(elemSearch)
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("there were unfulfilled expectations: %s", err)
		return
	}
...
func SelectByLoginAndPassword(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("cant create mock: %s", err)
	}
	...
	item, err := repo.SelectByLoginAndPassword(elem)
	...
	_, err = repo.SelectByLoginAndPassword(elem)
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("there were unfulfilled expectations: %s", err)
		return
	}
